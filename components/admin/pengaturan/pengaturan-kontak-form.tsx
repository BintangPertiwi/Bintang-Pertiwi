...
interface PengaturanKontakFormProps {
  globalConfig: Record<string, string>;
}

export function PengaturanKontakForm({ globalConfig }: PengaturanKontakFormProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isConfirmOpen, setIsConfirmOpen] = useState(false);

  const [headerTitle, setHeaderTitle] = useState(globalConfig["kontak_header_title"] || "Hubungi Kami");
  const [headerDesc, setHeaderDesc] = useState(globalConfig["kontak_header_desc"] || "Punya pertanyaan, masukan, atau ingin bekerja sama? Sampaikan langsung ke kontak person kami melalui WhatsApp.");
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      const payload = {
        kontak_header_title: headerTitle,
        kontak_header_desc: headerDesc,
      };
      ...
      toast.success("Pengaturan halaman kontak berhasil disimpan!");
    } catch (error) {
      console.error(error);
            ...
            <AccordionContent className="pt-4 pb-6 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder="Contoh: Punya pertanyaan atau ingin bekerja sama?..."
                  rows={3}
                  required
                />
              </div>
