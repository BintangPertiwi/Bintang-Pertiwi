...
export function ProdukLayout({ products, categories }: ProdukLayoutProps) {
  ...
  const filteredProducts = products.filter(product => {
    const matchesSearch = product.nama.toLowerCase().includes(deferredSearchQuery.toLowerCase());
  ...
  const currentListItems = filteredProducts.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);

  const isSearching = deferredSearchQuery.length > 0 || activeCategory !== "Semua" || inStockOnly;
        ...
        <div className="w-full lg:flex-1">
          ...
          {currentListItems.length > 0 ? (
            ...
            <div className="py-20 text-center flex flex-col items-center border border-dashed border-slate-200 rounded-2xl">
              ...
              <Button
                onClick={() => { setSearchQuery(""); setActiveCategory("Semua"); setInStockOnly(false); setCurrentPage(1); }}
                className="mt-6"
              >
                Reset Semua Filter
              </Button>
